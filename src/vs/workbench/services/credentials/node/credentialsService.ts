...
// TODO SHARE CODE WITH OTHER IMPLEMENTATION
export class KeytarCredentialsService extends Disposable implements ICredentialsService {
	...
	private static readonly PASSWORD_CHUNK_SIZE = KeytarCredentialsService.MAX_PASSWORD_LENGTH - 100;
	...
	async setPassword(service: string, account: string, password: string): Promise<void> {
		...
		if (isWindows && password.length > KeytarCredentialsService.MAX_PASSWORD_LENGTH) {
			let index = 0;
			let chunk = 0;
			let hasNextChunk = true;
			while (hasNextChunk) {
				const passwordChunk = password.substring(index, index + KeytarCredentialsService.PASSWORD_CHUNK_SIZE);
				index += KeytarCredentialsService.PASSWORD_CHUNK_SIZE;
