...
import { CompletionItem, Context as SuggestContext } from 'vs/editor/contrib/suggest/browser/suggest';
import { canExpandCompletionItem, SuggestDetailsOverlay, SuggestDetailsWidget } from 'vs/workbench/contrib/terminal/browser/suggest/suggestWidgetDetails';
import { ItemRenderer } from 'vs/workbench/contrib/terminal/browser/suggest/suggestWidgetRenderer';
...
class PersistedWidgetSize {
	...
	constructor(
		...
		this._key = `suggestWidget.size/terminal`;
	}
...
export class TerminalSuggestWidget implements IDisposable {
	...
	constructor(
		...
		this._contentWidget = new SuggestContentWidget(this/*, this._terminal*/);
		// TODO: Should this be per shell?
		this._persistedSize = new PersistedWidgetSize(_storageService);
		...
		// TODO: Support details properly
		const details = instantiationService.createInstance(SuggestDetailsWidget);
		details.onDidClose(this.toggleDetails, this, this._disposables);
		this._details = new SuggestDetailsOverlay(details, null!);

		// TODO: Setting?
		const applyIconStyle = () => this.element.domNode.classList.toggle('no-icons', false); //!this.editor.getOption(EditorOption.suggest).showIcons);
		applyIconStyle();

		const renderer = instantiationService.createInstance(ItemRenderer);
		...
		this._disposables.add(this._terminal.onData(() => this._onCursorSelectionChanged()));
	...
	private _setState(state: State): void {
		...
		switch (state) {
			...
			case State.Loading:
				...
				this._messageElement.textContent = TerminalSuggestWidget.LOADING_MESSAGE;
			...
			case State.Empty:
				this.element.domNode.classList.add('message');
				this._messageElement.textContent = TerminalSuggestWidget.NO_SUGGESTIONS_MESSAGE;
				dom.hide(this._listElement, this._status.element);
				dom.show(this._messageElement);
				this._details.hide();
				this._show();
	...
	showTriggered(auto: boolean, delay: number) {
		...
		this._contentWidget.setPosition({ column: 1, lineNumber: 1 });
	...
	private _layout(size: dom.Dimension | undefined): void {
		...
		if (this._state === State.Empty || this._state === State.Loading) {
			...
			const editorBox = dom.getDomNodePagePosition((this._terminal.xterm as any).raw.element);
			// const cursorBox = this.editor.getScrolledVisiblePosition(this.editor.getPosition());
			const cursorBox = { left: 0, top: 0, height: 16 };
	...
	getLayoutInfo() {
		...
		return {
			...
			borderWidth,
			borderHeight,
			typicalHalfwidthCharacterWidth: 12, // fontInfo.typicalHalfwidthCharacterWidth,
			verticalPadding: 22,
			horizontalPadding: 14,
			defaultSize: new dom.Dimension(430, 30 + 12 * 20 + borderHeight)
...
export class SuggestContentWidget implements IContentWidget {
	...
	constructor(
		private readonly _widget: TerminalSuggestWidget,
		// private readonly _terminal: ITerminalInstnace
	) { }
